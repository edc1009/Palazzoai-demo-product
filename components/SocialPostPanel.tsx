...
export const SocialPostPanel: React.FC<SocialPostPanelProps> = ({ appState, content }) => {
  const renderContent = () => {
    switch (appState) {
      ...
      case 'results_ready':
        return (
          <div className="h-full w-full p-1 whitespace-pre-wrap text-sm text-gray-800 overflow-y-auto">
            {content}
          </div>
        );
      case 'error':
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-lg border border-gray-200 min-h-[250px] flex items-center justify-center">
      {renderContent()}
    </div>
  );
};
